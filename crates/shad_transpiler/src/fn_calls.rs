...
use shad_analyzer::{Analysis, FnParam, Function};
use shad_parser::{
...
fn cast_fn_call(fn_: &Function, call: String) -> String {
    if fn_.return_type_id.as_deref() == Some("bool") {
        format!("u32({call})")
    } else {
        call
    }
...
fn cast_fn_arg(analysis: &Analysis, fn_: &Function, param: &FnParam, arg: &AstExpr) -> String {
    ...
    if fn_.ast.gpu_qualifier.is_some() && fn_.source_type.is_none() && type_.id == "bool" {
        format!("bool({expr})")
    } else {
        expr
    }
