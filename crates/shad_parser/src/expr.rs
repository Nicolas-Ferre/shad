...
impl AstExpr {
    ...
    pub(crate) fn parse(
        ...
        let mut expressions = vec![Self::parse_part(lexer, ids)?];
        ...
        loop {
            ...
            expressions.push(Self::parse_part(lexer, ids)?);
        }
        if expressions.len() == 1 {
            Ok(expressions.remove(0))
        } else {
            AstFnCall::parse_binary_operation(ids, &expressions, &operators).map(AstExpr::FnCall)
        }
    }
    ...
    pub(crate) fn parse_part(
        ...
        match token.type_ {
            ...
            TokenType::Minus => Ok(Self::FnCall(AstFnCall::parse_unary_operation(lexer, ids)?)),
            TokenType::Not => Ok(Self::FnCall(AstFnCall::parse_unary_operation(lexer, ids)?)),
